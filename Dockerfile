...
# 锁定 pnpm-lock.yaml，避免服务器构建时解析出不同依赖树。
...
# ---------- Stage 3: 运行时 ----------
FROM alpine:3.20
RUN apk add --no-cache bash ca-certificates curl docker-cli docker-cli-compose git openssh-client postgresql16-client su-exec tzdata wget && \
...
EXPOSE 8418
ENTRYPOINT ["/bin/sh", "-c", "mkdir -p /app/backups && chown upstream:upstream /app/backups && exec /app/upstream-hub"]
